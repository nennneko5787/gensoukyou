...
async def on_message(message: discord.Message):
	if message.type == discord.MessageType.default or message.type == discord.MessageType.reply:
		if message.author.bot == False:
			if discord.utils.find(lambda r: r.id == 1226065401650352148, message.mentions):
				embed = discord.Embed(
					title="このボットの使い方",
			...
			if discord.utils.find(lambda r: r.name == '博麗霊夢', message.role_mentions):
				await asyncio.create_task(博麗霊夢(message))
			if discord.utils.find(lambda r: r.name == '霧雨魔理沙', message.role_mentions):
